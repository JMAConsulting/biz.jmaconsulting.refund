 ...
 * @copyright CiviCRM LLC (c) 2004-2016
 */
class CRM_Refund_Form_Report_CreditCard extends CRM_Report_Form {
    ...
    $this->_columns = array(
      ...
      'civicrm_contribution' => array(
        ...
        'filters' => array(
          ...
          'receive_date' => array(
            'operatorType' => CRM_Report_Form::OP_DATE,
            'default' => array('receive_date_relative' => 'this.day'),
        ...
        'order_bys' => array(
          ...
          'receive_date' => array('title' => ts('Date Received using Closing Time')),
        ),
        'group_bys' => array(
          'receive_date' => array(
      ...
      'civicrm_contribution_cctype' => array(
        'dao' => 'CRM_Financial_DAO_FinancialTrxn',
        'alias' => 'cctype',
        'dbAlias' => 'civicrm_contribution_cctype.credit_card_type',
        'filters' => array(
          'credit_card_type' => array(
            'title' => ts('Credit Card Type'),
            'operatorType' => CRM_Report_Form::OP_MULTISELECT,
            'options' => CRM_Core_OptionGroup::values('accept_creditcard', FALSE, FALSE, FALSE, NULL, 'label', TRUE, FALSE, 'value'),
            'type' => CRM_Utils_Type::T_INT,
          ),
        ),
        'group_bys' => array(
  ...
  public function select() {
    ...
    $this->_select = str_replace("{$this->_aliases['civicrm_contribution']}.credit_card_type as civicrm_contribution_credit_card_type", "{$this->_aliases['civicrm_contribution_cctype']}.credit_card_type as civicrm_contribution_credit_card_type", $this->_select);
    $this->_selectClauses = str_replace("{$this->_aliases['civicrm_contribution']}.credit_card_type as civicrm_contribution_credit_card_type", "{$this->_aliases['civicrm_contribution_cctype']}.credit_card_type as civicrm_contribution_credit_card_type", $this->_selectClauses);
  }
  ...
  public function where() {
    ...
    $cc = array_search('Credit Card', $paymentInstruments);
    $this->_where .= " AND ( {$this->_aliases['civicrm_contribution']}.payment_instrument_id IN ({$cc}) )";
  }
   ...
   * @return array
   */
  public function getFromToFiscal($relative, $from, $to, $fromTime = NULL, $toTime = NULL) {
    if (empty($toTime)) {
      $toTime = '160000';
    }
    if (empty($fromTime)) {
  ...
  public function groupBy() {
    $groupBys = array("{$this->_aliases['civicrm_contact']}.id", "{$this->_aliases['civicrm_contribution']}.id");
...
(SELECT * FROM civireport_contribution_detail_temp2)";
  ...
  public function alterDisplay(&$rows) {
    ...
    $creditCard = CRM_Core_OptionGroup::values('accept_creditcard', FALSE, FALSE, FALSE, NULL, 'label', TRUE, FALSE, 'value');
    $paymentInstruments = CRM_Contribute_PseudoConstant::paymentInstrument();
    ...
    foreach ($rows as $rowNum => $row) {
      ...
      if ($value = CRM_Utils_Array::value('civicrm_contribution_credit_card_type', $row)) {
        $rows[$rowNum]['civicrm_contribution_credit_card_type'] = $creditCard[$value];
        $entryFound = TRUE;
      }
...
WHERE  civicrm_contribution_contribution_id={$row['civicrm_contribution_contribution_id']}";
  ...
  public function appendAdditionalFromJoins() {
      $this->_from .= "
              LEFT JOIN civicrm_entity_financial_trxn trxn
                      ON {$this->_aliases['civicrm_contribution']}.id = trxn.entity_id
                      AND trxn.entity_table = 'civicrm_contribution'
              LEFT JOIN civicrm_financial_trxn {$this->_aliases['civicrm_contribution_cctype']}
                      ON trxn.financial_trxn_id = {$this->_aliases['civicrm_contribution_cctype']}.id";
