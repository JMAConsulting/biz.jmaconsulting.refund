...
";
  ...
  public static function refundContribution($id, $params, $trxnID = NULL, $contribution) {
    if ($trxnID) {
      $result = civicrm_api3('FinancialTrxn', 'get', array('id' => $trxnID));
      $transactionID = NULL;
      if (isset($params['refund_trxn_id'])) {
        $transactionID = $params['refund_trxn_id'];
      }
      elseif (isset($result['values'][$result['id']]['trxn_id'])) {
        $transactionID = $result['values'][$result['id']]['trxn_id'];
      }
      if (isset($result['values'][$result['id']]['payment_processor_id']) && !empty($result['values'][$result['id']]['payment_processor_id'])) {
        $fromFinancialAccount = CRM_Financial_BAO_FinancialTypeAccount::getFinancialAccount($result['values'][$result['id']]['payment_processor_id'], 'civicrm_payment_processor', 'financial_account_id');
      }
      elseif (isset($params['payment_instrument_id']) && !empty($params['payment_instrument_id'])) {
        $fromFinancialAccount = CRM_Financial_BAO_FinancialTypeAccount::getInstrumentFinancialAccount($params['payment_instrument_id']);
      }
      else {
        $relationTypeId = key(CRM_Core_PseudoConstant::accountOptionValues('financial_account_type', NULL, " AND v.name LIKE 'Asset' "));
        $queryParams = array(1 => array($relationTypeId, 'Integer'));
        $fromFinancialAccount = CRM_Core_DAO::singleValueQuery("SELECT id FROM civicrm_financial_account WHERE is_default = 1 AND financial_account_type_id = %1", $queryParams);
      }
      ...
      $toFinancialAccount = CRM_Contribute_PseudoConstant::financialAccountType($contribution->_values['financial_type_id'], $relationTypeId);
      $trxnsData = array(
        'total_amount' => CRM_Utils_Rule::cleanMoney($result['values'][$result['id']]['total_amount']),
        'net_amount' => CRM_Utils_Rule::cleanMoney($result['values'][$result['id']]['total_amount']),
        'trxn_date' => date('YmdHis', strtotime($params['cancel_date'] . ' ' . $params['cancel_date_time'])),
        'payment_instrument_id' => empty($params['payment_instrument_id']) ? $result['values'][$result['id']]['payment_instrument_id'] : $params['payment_instrument_id'],
        'trxn_id' => $transactionID,
        'currency' => $result['values'][$result['id']]['currency'],
        'check_number' => $params['check_number'],
        'status_id' => CRM_Core_OptionGroup::getValue('contribution_status', 'Refunded', 'name'),
        'from_financial_account_id' => $fromFinancialAccount,
        'to_financial_account_id' => $toFinancialAccount,
      );
      $accounts = array();
    }
    else {
      $relationTypeId = key(CRM_Core_PseudoConstant::accountOptionValues('account_relationship', NULL, " AND v.name LIKE 'Credit/Contra Revenue Account is' "));
      $toFinancialAccount = CRM_Contribute_PseudoConstant::financialAccountType($contribution->_values['financial_type_id'], $relationTypeId);
      ...
      $relationTypeId = key(CRM_Core_PseudoConstant::accountOptionValues('account_relationship', NULL, " AND v.name LIKE 'Accounts Payable Account is' "));
      $fromFinancialAccount = CRM_Contribute_PseudoConstant::financialAccountType($contribution->_values['financial_type_id'], $relationTypeId);
      $trxnsData = array(
        'total_amount' => CRM_Utils_Rule::cleanMoney($params['refund_amount']),
        'net_amount' => CRM_Utils_Rule::cleanMoney($params['refund_amount']),
        'trxn_date' => date('YmdHis', strtotime($params['cancel_date'] . ' ' . $params['cancel_date_time'])),
        'currency' => $params['currency'],
        'status_id' => CRM_Core_OptionGroup::getValue('contribution_status', 'Pending refund', 'name'),
        'from_financial_account_id' => $fromFinancialAccount,
        'is_payment' => FALSE,
      );
      $accounts = array(
...
";
  ...
  public static function recordFinancialAccounts(&$params, $financialTrxnValues = NULL) {
    ...
    if ($contributionStatus == 'Partially paid'
      ...
      if (empty($balanceTrxnInfo['trxn_id'])) {
        ...
        $balanceTrxnParams['payment_instrument_id'] = empty($params['payment_instrument_id']) ? $params['contribution']->payment_instrument_id : $params['payment_instrument_id'];
  ...
  public static function checkStatusValidation($values, &$fields, &$errors) {
    ...
    $checkStatus = array(
      ...
      'Completed' => array('Cancelled', 'Refunded', 'Chargeback', 'Pending refund'),
      ...
      'Pending refund' => array('Refunded', 'Partially refunded'),
...
";
    ...
    elseif ($paymentType == 'refund') {
      ...
      $params['skipLineItem'] = TRUE;
      $trxnsData['trxn_date'] = !empty($trxnsData['trxn_date']) ? $trxnsData['trxn_date'] : date('YmdHis');
      if (CRM_Utils_Array::value('payment_instrument_id', $trxnsData)) {
      ...
      if (!empty($accounts['from'])) {
        $fromRelationTypeId = key(CRM_Core_PseudoConstant::accountOptionValues('account_relationship', NULL, " AND v.name LIKE '{$accounts['from']}' "));
        $trxnsData['from_financial_account_id'] = CRM_Contribute_PseudoConstant::financialAccountType($contributionDAO->financial_type_id, $fromRelationTypeId);
      }
      if ($trxnsData['status_id'] == CRM_Core_OptionGroup::getValue('contribution_status', 'Pending refund', 'name')) {
        $trxnsData['to_financial_account_id'] = NULL;
        $toFinancialAccountId = $accounts['to'];
        $refundStatus = CRM_Core_OptionGroup::getValue('contribution_status', 'Refunded', 'name');
      }
      $trxnsData['status_id'] = !empty($trxnsData['status_id']) ? $trxnsData['status_id'] : CRM_Core_OptionGroup::getValue('contribution_status', 'Refunded', 'name');
      // record the entry
      $financialTrxn = CRM_Contribute_BAO_Contribution::recordFinancialAccounts($params, $trxnsData);
      ...
      if (!empty($lineItems)) {
        foreach ($lineItems as $lineItemId => $lineItemValue) {
          ...
          if ($lineItemValue['financial_type_id']) {
            $searchParams = array(
              'entity_table' => 'civicrm_financial_type',
              'entity_id' => $lineItemValue['financial_type_id'],
    ...
    // activity creation
    if (!empty($financialTrxn) && $participantId) {
  ...
  public static function getPaymentInfo($id, $component, $getTrxnInfo = FALSE, $usingLineTotal = FALSE) {
    ...
    $rows = array();
    $pendingRefundStatus = CRM_Core_OptionGroup::getValue('contribution_status', 'Pending refund', 'name');
    if ($getTrxnInfo && $baseTrxnId) {
      ...
      $sql = "
        ...
        FROM civicrm_contribution con
          ...
          LEFT JOIN civicrm_financial_trxn pft ON pft.id = peft.financial_trxn_id
      ...
      while ($resultDAO->fetch()) {
        ...
        if ($val['status'] == 'Pending refund' && $resultDAO->entity_id != $resultDAO->ft_trxn_id) {
          $val['link'] = CRM_Utils_System::url('civicrm/contact/view/contribution',
            "reset=1&action=revert&cid={$resultDAO->contact_id}&id={$resultDAO->contribution_id}&trxnId={$resultDAO->ft_trxn_id}");
        }
        $rows[] = $val;
      }
