...
class CRM_Contribute_Form_Contribution extends CRM_Contribute_Form_AbstractEditPayment {
  ...
  public function preProcess() {
    ...
    $this->_trxnID = CRM_Utils_Request::retrieve('trxnId', 'Positive', $this);

    $this->_isSubmitted = CRM_Utils_Request::retrieve('isSubmitted', 'Positive', $this);

    $this->assign('trxnID', $this->_trxnID);

    $this->assign('isSubmitted', $this->_isSubmitted);
  ...
  public function setDefaultValues() {
    ...
    // Set default for refund amount.
    if ($this->_action & CRM_Core_Action::REVERT) {
      $defaults['refund_amount'] = $defaults['total_amount'];
  ...
  public function buildQuickForm() {
    ...
    if ($this->_action & CRM_Core_Action::REVERT) {
      if (!empty($this->_trxnID)) {
        if ($this->_isSubmitted) {
          $message = "Successfully recorded a pending refund!
            <br/>You can either proceed to process the refund, or press Cancel to go back to process the refund at a later time.";
          CRM_Core_Session::setStatus(ts($message), ts('Success!'), 'success');
        }
        CRM_Utils_System::setTitle(ts('Process Refund'));
        $this->addDateTime('cancel_date', ts('Refunded Date'), FALSE, array('formatType' => 'activityDateTime'));
      ...
      CRM_Utils_System::setTitle(ts('Create a Pending Refund'));
      $this->add('textarea', 'cancel_reason', ts('Refund Reason'), $attributes['cancel_reason']);
      $this->addDateTime('cancel_date', ts('Refunded Date'), FALSE, array('formatType' => 'activityDateTime'));
      ...
      $this->addButtons(array(
          array(
            'type' => 'upload',
            'name' => ts('Create a Pending Refund'),
            'spacing' => '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;',
            'isDefault' => TRUE,
            'subName' => 'revert',
          ),
  ...
  public function postProcess() {
    ...
    elseif ($buttonName == $this->getButtonName('upload', 'revert')) {
      CRM_Utils_System::redirect(CRM_Utils_System::url('civicrm/contact/view/contribution',
        "reset=1&action=revert&context=processrefund&cid={$this->_contactID}&trxnId={$this->_trxnId}&id={$this->_id}&isSubmitted=1"
      ));
    }
  ...
  protected function submit($submittedValues, $action, $pledgePaymentID) {
    ...
    if ($action & CRM_Core_Action::REVERT) {
      if (!empty($this->_trxnID)) {
        CRM_Contribute_BAO_Contribution::refundContribution($this->_id, $submittedValues, $this->_trxnID, $this);
        CRM_Core_Session::singleton()->replaceUserContext(CRM_Utils_System::url('civicrm/contact/view',
          "reset=1&cid={$this->_contactID}&selectedChild=contribute"
        ));
        return;
      }
      $trxn = CRM_Contribute_BAO_Contribution::refundContribution($this->_id, $submittedValues, NULL, $this);
      $this->_trxnId = $trxn->id;
      return;
    }
