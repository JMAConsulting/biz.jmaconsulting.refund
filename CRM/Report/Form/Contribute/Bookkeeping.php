...
class CRM_Report_Form_Contribute_Bookkeeping extends CRM_Report_Form {
          ...
          if (!empty($field['required']) ||
            ...
            switch ($fieldName) {
              ...
              case 'debit_accounting_code':
                $select[] = " CASE
                            WHEN {$this->_aliases['civicrm_financial_trxn']}.to_financial_account_id IS NOT NULL
                            THEN  {$this->_aliases['civicrm_financial_account']}_debit_1.accounting_code
                            ELSE  {$this->_aliases['civicrm_financial_account']}_debit_2.accounting_code
                            END AS civicrm_financial_account_debit_accounting_code ";
                break;
              ...
              case 'debit_name':
                $select[] = " CASE
                            WHEN {$this->_aliases['civicrm_financial_trxn']}.to_financial_account_id IS NOT NULL
                            THEN  {$this->_aliases['civicrm_financial_account']}_debit_1.name
                            ELSE  {$this->_aliases['civicrm_financial_account']}_debit_2.name
                            END AS civicrm_financial_account_debit_name ";
                break;
  ...
  public function from() {
    ...
    $this->_from = "FROM  civicrm_contact {$this->_aliases['civicrm_contact']} {$this->_aclFrom}
              ...
              LEFT JOIN civicrm_financial_account {$this->_aliases['civicrm_financial_account']}_debit_1
                    ON {$this->_aliases['civicrm_financial_trxn']}.to_financial_account_id = {$this->_aliases['civicrm_financial_account']}_debit_1.id
              ...
              LEFT JOIN civicrm_financial_account {$this->_aliases['civicrm_financial_account']}_debit_2
                    ON fitem.financial_account_id = {$this->_aliases['civicrm_financial_account']}_debit_2.id
    ...
    foreach ($this->_columns as $tableName => $table) {
      if (array_key_exists('filters', $table)) {
        foreach ($table['filters'] as $fieldName => $field) {
          ...
          elseif ($fieldName == 'debit_accounting_code') {
            $field['dbAlias'] = "CASE
              WHEN financial_trxn_civireport.to_financial_account_id IS NOT NULL
              THEN  financial_account_civireport_debit_1.accounting_code
              ELSE  financial_account_civireport_debit_2.accounting_code
              END";
          }
          ...
          elseif ($fieldName == 'debit_name') {
            $field['dbAlias'] = "CASE
              WHEN financial_trxn_civireport.to_financial_account_id IS NOT NULL
              THEN  financial_account_civireport_debit_1.id
              ELSE  financial_account_civireport_debit_2.id
              END";
          }
