...
<div class="action-link">
  <div class="crm-submit-buttons">
    {if (call_user_func(array('CRM_Core_Permission','check'), 'edit contributions') && call_user_func(array('CRM_Core_Permission', 'check'), "edit contributions of type $financial_type") && $canEdit) ||
      ...
      {assign var='urlRefundParams' value="reset=1&id=$id&cid=$contact_id&action=revert&context=$context"}
      ...
      {if $contribution_status eq 'Completed'}
        {if ( $context eq 'fulltext' || $context eq 'search' ) && $searchKey}
          {assign var='urlRefundParams' value="reset=1&id=$id&cid=$contact_id&action=revert&context=$context&key=$searchKey"}
        {/if}
        <a class="button" href="{crmURL p='civicrm/contact/view/contribution' q=$urlRefundParams}" accesskey="e"><span>
          <i class="crm-i fa-refresh fa-spin"></i> {ts}Refund{/ts}</span>
        </a>
      {/if}
    {/if}
  ...
  {if (call_user_func(array('CRM_Core_Permission','check'), 'edit contributions') && call_user_func(array('CRM_Core_Permission', 'check'), "edit contributions of type $financial_type") && $canEdit) ||
    ...
    {if $contribution_status eq 'Completed'}
      {if ( $context eq 'fulltext' || $context eq 'search' ) && $searchKey}
        {assign var='urlRefundParams' value="reset=1&id=$id&cid=$contact_id&action=revert&context=$context&key=$searchKey"}
      {/if}
      <a class="button" href="{crmURL p='civicrm/contact/view/contribution' q=$urlRefundParams}" accesskey="e"><span><i class="crm-i fa-refresh fa-spin"></i> {ts}Refund{/ts}</span></a>
    {/if}
  {/if}
  {if (call_user_func(array('CRM_Core_Permission','check'), 'delete in CiviContribute') && call_user_func(array('CRM_Core_Permission', 'check'), "delete contributions of type $financial_type") && $canDelete)     || (call_user_func(array('CRM_Core_Permission','check'), 'delete in CiviContribute') && $noACL)}
    {assign var='urlParams' value="reset=1&id=$id&cid=$contact_id&action=delete&context=$context"}
